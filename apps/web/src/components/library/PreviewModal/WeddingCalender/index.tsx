...
const WeddingCalender = ({ data }: Props) => {
  ...
  const { date, weddingHallName: name, isCalendarVisible } = data.weddingInfo;
  ...
  return (
    <Wrapper>
        ...
        <>
          ...
          <Grid>
            {['일', '월', '화', '수', '목', '금', '토'].map((wd) => (
              <Cell style={{ fontFamily: invitationFont }} key={wd}>
                {wd}
              </Cell>
            ))}
            {cells.map((num, i) => (
              <Cell
                ...
                style={{ fontFamily: invitationFont }}
              >
                {num || ''}
              </Cell>
            ))}
      ...
      <Column>
        <Info style={{ fontFamily: invitationFont }}>
